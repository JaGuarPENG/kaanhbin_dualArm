...
namespace rivet
{
    ...
    auto Arm2Init::executeRT()->int {
        ...
        {
            double temp_pos[6]{-0.622364, -0.206756, 0.08, PI, 0, PI};
            std::copy(temp_pos, temp_pos+6, assem_pos);
        }
        else
        {
    ...
    struct RivetSearch::Imp {
        ...
        double phase5_fd[6]{ 0,0,5,0,0,0 };
        ...
        double phase5_B[6]{ 3000,3000,3500,0,0,0 };
        ...
        double p5_deadzone[6]{3.0,3.0,0.1,0,0,0};
    ...
    auto RivetSearch::executeRT() -> int
    {
        ...
        auto boltJammedCheck = [&](double net_disp_, int count_)
        {
            ...
            if (abs(imp_->check_disp - net_disp_) >= 0.00015)
            {
                isValid = false;
            }
...
//                                    << "Deg:"<<'\t'<< 57.296 * imp_->force_direction[imp_->search_counter] <<std::endl;
    ...
    struct RivetHoleDection::Imp {
        ...
        //Desired Force of Each Phase
        double phase2_fd[6]{ 0 };
        double phase3_fd[6]{ 0,0,8,0,0,0 };
        double phase4_fd[6]{ 0,0,5,0,0,0 };
        double phase5_fd[6]{ 0,0,8,0,0,0 };
        ...
        double p5_deadzone[6]{2,2,0.1,0,0,0};
        double p6_deadzone[6]{3,3,3,0,0,0};
        ...
        else
        {
            ...
            else if (imp_->phase2 && !imp_->phase3)
            {
                ...
                if (posCheck(arm2_current_pos, 5))
                {
                    ...
                    // master()->logFileRawName("Search_contact");
                    master()->logFileRawName(std::string("/home/kaanh/Desktop/kaanhbin/rivet_hd_test/rivetTest_" + aris::core::logFileTimeFormat(std::chrono::system_clock::now())).c_str());
                    imp_->total_search_count = count();
                ...
                else
                {
                    ...
                    if(arm2_final_force[2] >= (imp_->phase3_fd[2] - 3.0) && arm2_final_force[2] <= (imp_->phase3_fd[2] + 2.0))
                    {
                        arm2_final_force[2] = imp_->phase3_fd[2];
                    }
            ...
            else if (imp_->phase3 && !imp_->phase4)
            {
                ...
                else
                {
                    ...
                    if(imp_->search_counter < 30)
                    {
                        ...
                        imp_->force_direction[imp_->search_counter] = (1 - abs(imp_->avg_trans_x_force / 20.0))*std::atan2((imp_->py[imp_->search_counter+1] - imp_->py[imp_->search_counter]), (imp_->px[imp_->search_counter+1] - imp_->px[imp_->search_counter])) - imp_->theta;
